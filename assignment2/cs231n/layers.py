...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    D = np.prod(x.shape[1:])  # multiply all dimensions except the first
    x_reshaped = x.reshape(x.shape[0], D)  # matrix is now N x D
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, gamma, eps, batch_mean, batch_var, norm_x)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    #using version of full derivatives here instead of computation graph like computations
    
    x, gamma, eps, batch_mean, batch_var, norm_x = cache
    
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * norm_x, axis=0)
    dnorm_x = dout * gamma
    
    dbatch_var = np.sum(dnorm_x * (x - batch_mean) * (-1/2) * ((batch_var + eps) ** (-3/2)), axis=0)
    dbatch_mean = np.sum(dnorm_x * (-1) / np.sqrt(batch_var + eps), axis=0) \
                  + dbatch_var * np.sum(-2 * (x - batch_mean), axis=0) / dout.shape[0]
    dx = dnorm_x * 1 / (np.sqrt(batch_var + eps)) + dbatch_var * 2 * (x - batch_mean) / dout.shape[0] \
         + dbatch_mean / dout.shape[0]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        out = x  # nothing needs to be done here
        #######################################################################
        #                            END OF YOUR CODE                         #
        #######################################################################
        ...
        for f in range(F):
            ...
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    out[n, f, i, j] = np.sum(x_padded[n, :, start_H:end_H, start_W:end_W]
                                             * w[f, :, :, :]) + b[f]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_reduced = x.swapaxes(1, 3).reshape(N*W*H, C)
    out_reduced, cache = batchnorm_forward(x_reduced, gamma, beta, bn_param)
    out = out_reduced.reshape(N, W, H, C).swapaxes(1, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_reduced = dout.swapaxes(1, 3).reshape(N*W*H, C)
    dx_reduced, dgamma, dbeta = batchnorm_backward(dout_reduced, cache)
    dx = dx_reduced.reshape(N, W, H, C).swapaxes(1, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
