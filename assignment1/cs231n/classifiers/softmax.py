...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    scores = X[i].dot(W)
    scores -= np.max(scores)  # shift values to avoid numerical instabilities
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  correct_class_score = norm_scores[np.arange(num_train), y]
  loss = np.sum(-np.log(correct_class_score))
    
  ind = np.zeros(norm_scores.shape)
  ind[np.arange(num_train), y] = 1
  dW = X.T.dot(norm_scores - ind)
